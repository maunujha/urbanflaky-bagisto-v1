...
**Overall Phase 5 conclusion: zero regressions found from any phase of this cleanup.** All non-pre-existing-pattern test failures were individually traced to root cause and confirmed unrelated via direct `git diff` against `dev`.

**Bonus fixes (user-approved, 2026-06-21):** the two pre-existing `Gabha\RewardCoins` bugs found above were fixed:
1. Added the missing `TransactionStatus::Reversed` enum case (`packages/Gabha/RewardCoins/src/Enums/TransactionStatus.php`), updated its `label()`/`isTerminal()` match arms, added the `reward_coins.transaction.statuses.reversed` lang key, and a status-badge color for it in `transaction-row.blade.php`. `reverseForOrder()` in `CoinTransactionRepository` now completes instead of fatally erroring.
2. The Closure-serialization crash turned out to be **systemic**, not specific to one listener: investigation proved any `ShouldQueue` listener on `sales.order.update-status.after` crashes when Laravel tries to serialize the queued job for `$order` — confirmed by the fact that fixing only `ReverseCoinsOnCancellation` left `ConfirmCoinsOnDelivery` (the other listener sharing that event) still crashing identically. Removed `ShouldQueue`/`InteractsWithQueue` from **all three** RewardCoins order-event listeners (`AwardCoinsOnOrder`, `ConfirmCoinsOnDelivery`, `ReverseCoinsOnCancellation`), each now running synchronously with the original `failed()`-callback logging folded into an inline `try/catch` to preserve the same fail-safe behavior. The root cause (something in the order/relation graph that isn't serializable) lives in core Bagisto, not this package — out of scope to chase further, and moot now that these listeners no longer go through serialization at all.

Verified: `php -l` clean on all 4 edited files, `optimize:clear` clean. Re-ran the Admin Sales+Catalog suites: failures dropped from 9 to 5, with the 2 RewardCoins-specific failures (the enum crash and the Closure-serialization crash) now gone; the remaining 5 are the same already-documented pre-existing patterns (carrier-config test gap, payment-method-count staleness, multi-line-address assertion quirk, invoice currency-formatter null-argument). Confirmed RewardCoins is `is_active=true` in this environment, so these were live, exploitable bugs, not dormant behind a disabled flag.
